...
SELECT CAST (nextval('seq_hadoop_loanproduct') AS INT) AS sequenceid, *
...
SELECT CAST (nextval('seq_hadoop_users') AS INT) AS sequenceid, *
FROM public.users;
...
idnumber, devicetoken, deviceostype, mnoiprsmatchpercentage, hashkey, riskrating, registrationchannelid, remindersent, temppin, preapprovalstatus,
postapprovalstatus, netrevielprimarystatus, netrevielscreenstatus, netrevielscreenapprovedstatus, lastmodifiedbyipaddress, userloanstatus, loanrestrictduedate, 
migrationstatus, checkforcechangepassword, rolesubtype, loanstatusdate
